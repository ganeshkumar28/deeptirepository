				...
				<tbody>
					<c:forEach var="product" items="${fav_list}">
						<tr>
							...
							<td>${product.favoriteId}</td>
							<td>${product.productName}</td>
							<td class="product-description">${product.productDescription}</td>
							<td>${product.diecountPrice}</td>
							<td>
								<form action="${pageContext.request.contextPath}/buyer/removeFromFavorite" method="post">
									<input type="hidden" name="favoriteId" value="${product.favoriteId}">
